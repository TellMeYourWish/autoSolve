// ==UserScript==
// @name       				OCS 网课助手（autoSolve 本地适配版）
// @version    				4.15.6.1
// @description				基于 OCS MIT 版本的非官方本地适配：接入 autoSolve 与超星字体解码，默认只保存、不自动提交。仅用于获授权的学习辅助、无障碍访问与兼容性测试。
// @author     				enncy (upstream), autoSolve adapter contributors
// @license    				MIT
// @match      				*://*.zhihuishu.com/*
...
// @namespace  				urn:autosolve:ocs-adapter
...
(function(global2, factory) {
  ...
  class OCSWorker extends lib.CommonEventEmitter {
    ...
    uploadHandler(options) {
      ...
      const safeMode = CommonProject && CommonProject.scripts.settings.cfg.safeMode !== false;
      const effectiveType = safeMode && type !== "nomove" ? "save" : type;
      if (effectiveType !== "nomove") {
        let finished = 0;
        ...
        if (effectiveType === "force") {
          ...
          return callback(rate, effectiveType === "save" ? false : rate >= parseFloat(effectiveType.toString()));
        }
      }
    }
  }
...
`;
  ...
  const AUTOSOLVE_DEFAULT_OCS_URL = "http://127.0.0.1:8080/api/ocs/solve";
  const AUTOSOLVE_FONT_DECODE_PATH = "/api/ocs/font/decode";
  function buildAutoSolveFontDecodeUrl(rawUrl) {
    const url = String(rawUrl || AUTOSOLVE_DEFAULT_OCS_URL).trim();
    let parsed;
    try {
      parsed = new URL(url);
    } catch {
      throw new Error("autoSolve font decoder URL is invalid.");
    }
    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
      throw new Error("autoSolve font decoder only supports http(s).");
    }
    return new URL(AUTOSOLVE_FONT_DECODE_PATH, parsed.origin).toString();
  }
  function buildAutoSolveAnswererWrapper(rawUrl) {
    const url = String(rawUrl || AUTOSOLVE_DEFAULT_OCS_URL).trim();
    let parsed;
    try {
      parsed = new URL(url);
    } catch {
      throw new Error("autoSolve 接口地址无效，请检查通用-全局设置中的地址。");
    }
    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
      throw new Error("autoSolve 接口只支持 http 或 https 地址。");
    }
    return {
      name: "autoSolve（OCS 接口）",
      ...
      handler: "return (res)=>{if(!res||res.code!==1)throw new Error((res&&res.msg)||'autoSolve 返回失败');const item=res.data&&res.data[0];return item&&item.answer?[[item.question||'',item.answer,{confidence:item.confidence,explanation:item.explanation,request_id:res.request_id}]]:[]}"
    };
  }
      ...
      settings: new lib.Script({
        ...
        configs: {
          ...
          answerApiMode: {
            label: "答题接口",
            tag: "select",
            defaultValue: "autosolve",
            options: [
              ["autosolve", "autoSolve（推荐）", "使用本机 autoSolve 的 OCS 专用接口"],
              ["custom", "OCS 原题库配置", "继续使用下方题库配置中的接口"]
            ]
          },
          autoSolveApiUrl: {
            label: "autoSolve OCS 接口",
            defaultValue: AUTOSOLVE_DEFAULT_OCS_URL,
            attrs: {
              type: "text",
              title: "默认：http://127.0.0.1:8080/api/ocs/solve"
            }
          },
          ...
          safeMode: {
            ...
            attrs: {
              type: "checkbox",
              title: "开启后无论完成率和提交选项如何，章节测试都只保存、不提交。"
            }
          },
          thread: {
            label: "线程数量（个）",
        ...
        methods() {
          return {
            getWorkOptions: () => {
              ...
              if (workOptions.answerApiMode === "autosolve") {
                workOptions.answererWrappers = [buildAutoSolveAnswererWrapper(workOptions.autoSolveApiUrl)];
              }
              if (workOptions.safeMode !== false) {
                workOptions.upload = "save";
              }
              return workOptions;
            },
        ...
        onrender({ panel }) {
          if (lib.$gm.isInGMContext()) {
            const effectiveWrappers = () => this.methods.getWorkOptions().answererWrappers;
            panel.body.replaceChildren(...effectiveWrappers().length ? [lib.h("hr")] : []);
            const testNotification = lib.h(
              "button",
              { className: "base-style-button", disabled: effectiveWrappers().length === 0 },
              "📢测试系统通知"
            );
            ...
            const updateState = async () => {
              ...
              let loadedCount = 0;
              const wrappers = effectiveWrappers();
              if (wrappers.length) {
                ...
                wrappers.forEach(async (item) => {
                  ...
                  table.append(body);
                  loadedCount++;
                  if (loadedCount === wrappers.length) {
                    setTimeout(() => {
      ...
      onlineSearch: new lib.Script({
        ...
        onrender({ panel }) {
          ...
          const search = async (value) => {
            const answererWrappers = CommonProject.scripts.settings.methods.getWorkOptions().answererWrappers;
            if (answererWrappers.length === 0) {
            ...
            if (value) {
              const t2 = Date.now();
              const infos = await defaultAnswerWrapperHandler(answererWrappers, {
                title: value
              });
...
主页：${c.homepage || "未知主页"}`,
  ...
  const CXProject = lib.Project.create({
    ...
    scripts: {
      ...
      study: new lib.Script({
        ...
        onrender({ panel }) {
          if (!CommonProject.scripts.settings.methods.getWorkOptions().answererWrappers.length) {
  ...
  async function mappingRecognize(doc = document) {
    if (!doc.querySelector(".font-cxsecret")) {
      return;
    }
    ...
    const font = fontFaceEl == null ? void 0 : fontFaceEl.textContent.match(/base64,([A-Za-z0-9+/=]+)/)?.[1];
    if (!font) {
      throw new Error("Chaoxing secret font was not found; answering has been stopped.");
    }
    const fonts = CXAnalyses.getSecretFont(doc);
    const usedCodepoints = Array.from(
      new Set(fonts.flatMap((el) => Array.from(el.textContent || "").map((word) => word.codePointAt(0))))
    );
    let response;
    try {
      const rawUrl = CommonProject.scripts.settings.cfg.autoSolveApiUrl;
      response = await request(buildAutoSolveFontDecodeUrl(rawUrl), {
        type: "GM_xmlhttpRequest",
        method: "post",
        responseType: "json",
        headers: { "Content-Type": "application/json" },
        data: { font_base64: font, used_codepoints: usedCodepoints }
      });
    } catch (err) {
      $console.error("autoSolve font decoder is unavailable:", String(err));
      throw new Error("autoSolve font decoder is unavailable; answering has been stopped.", { cause: err });
    }
    const data = response == null ? void 0 : response.data;
    if (!response || response.code !== 1 || !data || data.complete !== true || data.unresolved_codepoints.length !== 0) {
      throw new Error((response == null ? void 0 : response.msg) || "Chaoxing font decoding was incomplete; answering has been stopped.");
    }
    const replacements = Object.entries(data.mappings || {}).map(([source, target]) => [
      String.fromCodePoint(Number(source)),
      String.fromCodePoint(Number(target))
    ]);
    if (data.used_font_codepoints.length > 0 && replacements.length === 0) {
      throw new Error("Chaoxing font mapping was empty; answering has been stopped.");
    }
    fonts.forEach((el) => {
    ...
    doc.querySelectorAll(".font-cxsecret").forEach((el) => el.classList.remove("font-cxsecret"));
    if (doc.querySelector(".font-cxsecret")) {
      throw new Error("Chaoxing font decoding did not finish; answering has been stopped.");
    }
    $console.log(`Chaoxing font decoded by autoSolve (${replacements.length} glyphs).`);
  }
  const CXAnalyses = {
    isInSpecialMode() {
  ...
  const JobRunner = {
    ...
    async chapter(frame, {
      ...
      await mappingRecognize(frameWindow.document);
      $console.info("开始章节测试");
